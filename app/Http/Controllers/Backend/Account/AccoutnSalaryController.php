...
class AccoutnSalaryController extends Controller
{
   ...
   public function AccountSalaryGetEmployee(Request $request){
    ...
    foreach ($data as $key => $attend) {
        ...
        $accoutn_salary = AccountEmployeeSalary::where('employee_id', $attend->employee_id)->where('date', $date)->first();


      if($accoutn_salary != null){
        $checked = 'checked';
        ...
        $html[$key]['tdsource'] .= '<td>'.$attend['user']['id_no'].'<input name="date" type="hidden" value="' . $date . '"  >'.'</td>';
   ...
   public function AccountSalaryStore(Request $request){
    ...
    if(!empty($date) || empty($checkdata)){
        $notification = array(
            'message' => 'Well Done Data Successfully Update',
            'alert-type' => 'success'
        );
