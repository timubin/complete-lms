...
class ExamFeeController extends Controller
{
        ...
        public function ExamFeeClassData(Request $request){
            ...
            foreach ($allStudent as $key => $v) {
                ...
                // Check if $registrationfee is null
                if ($registrationfee !== null) {
                    ...
                    $html[$key]['tdsource'] .='<td>'.$finalfee.'TK'.'</td>'; // doller and taka simble 
                    $html[$key]['tdsource'] .='<td>';
                    $html[$key]['tdsource'] .='<a class="btn btn-sm btn-'.$color.'" title="PaySlip" target="_blanks" href="'.route("student.exam.fee.payslip").'?class_id='.$v->class_id.'&student_id='.$v->student_id.'&exap_type_id='.$request->exap_type_id.'">Fee Slip</a>';
                    $html[$key]['tdsource'] .= '</td>';
                }
            }
       ...
       public function ExamFeePayslip(Request $request){
        ...
        if ($examType) {
            $data['exam_type'] = $examType->name;
        } else {
            $data['exam_type'] = 'Unknown';
        }
